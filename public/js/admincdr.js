...
if (!token || (role !== 'compras' && role !== 'admincdr')) {
...
function renderTickets(tickets) {
    ...
    <table class="tickets-table">
      <thead>
        <tr>
          ...
          <th class="center-col">ELIMINAR</th> <!-- Nueva columna para el botón -->
  ...
  visibleTickets.forEach(ticket => {
    ...
    row.innerHTML = `
      ...
      <td class="center-col">
        <!-- Botón de eliminar -->
        <button class="delete-ticket-btn" data-id="${ticket._id}" style="padding: 6px 10px;">
          Eliminar
        </button>
      </td>
    `;
  ...
  deleteButtons.forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation(); // Evita que se dispare el evento de abrir modal, si lo hay
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(errData.error || 'Error al eliminar el ticket');
        }
        alert('Ticket eliminado con éxito');
        // Vuelve a cargar la lista de tickets
        fetchAllTickets(); 
      } catch (error) {
        console.error('Error al eliminar ticket:', error);
        alert(error.message || 'Error al eliminar el ticket');
      }
    });
  });   
...
if (tituloCompras && username) {
  // Puedes distinguir según el rol:
  tituloCompras.textContent = (role === 'admincdr' 
    ? `ADMIN CDR - Tickets` 
    : `${username.toUpperCase()} - Tickets`);
}
